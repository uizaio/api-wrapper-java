...
public class Analytic extends ApiResource {
   ...
   * Track video playback on any metric performance, so you can know exactly what’s happening on
   * every user’s device and debug more effectively.
   *
  ...
  public static JsonArray getTotalLine(Map<String, Object> lineParams) throws UizaException {
    String path_extension = String.format("%s/%s", CLASS_DEFAULT_PATH, TOTAL_LINE_PATH);
    JsonElement response = request(RequestMethod.GET, buildRequestURL(path_extension), lineParams);
  ...
  public static JsonArray getType(Map<String, Object> lineParams) throws UizaException {
    String path_extension = String.format("%s/%s", CLASS_DEFAULT_PATH, TYPE_PATH);
    JsonElement response = request(RequestMethod.GET, buildRequestURL(path_extension), lineParams);
  ...
  /**
   * Get data grouped by hour. Get total view in time range.
  ...
  public static JsonArray getLine(Map<String, Object> lineParams) throws UizaException {
    String path_extension = String.format("%s/%s", CLASS_DEFAULT_PATH, LINE_PATH);
    JsonElement response = request(RequestMethod.GET, buildRequestURL(path_extension), lineParams);
